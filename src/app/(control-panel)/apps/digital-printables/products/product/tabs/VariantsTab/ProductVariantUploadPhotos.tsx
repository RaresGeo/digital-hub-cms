							...
							onRemove={async () => {
								const newPhotos = photos.map((photo) => {
									return {
										...photo,
										sortOrder:
											photo.sortOrder > photoToRemove.sortOrder
												? photo.sortOrder - 1
												: photo.sortOrder
									};
								});
								...
								setValue(
									...
									newPhotos.filter((photo) => photo.id !== photoId)
								);
		...
		<div className="flex justify-center sm:justify-start flex-wrap -mx-3">
					...
					>
						<input
							...
							onChange={async (e) => {
								...
								onChange([
									{
										...(newImage as { url: string; id: string }),
										sortOrder: Number(value?.length ?? 0)
									},
									...((value ?? []) as DigitalPrintableProductVariant['photos'])
								]);
							}}
						/>
						<FuseSvgIcon
			...
			<Controller
				...
				render={({ field: { onChange, value } }) => {
					return (
						<>
							{variants[variantIndex].photos.map((media) => (
								<Box
									sx={(theme) => ({
										backgroundColor: lighten(theme.palette.background.default, 0.02),
										...theme.applyStyles('light', {
								...
								>
									...
									<FuseSvgIcon
										onClick={() => handleOnRemovePhoto(media.id)}
